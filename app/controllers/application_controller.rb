...
class ApplicationController < ActionController::Base
  ...
  def handle_generic_exception(exception, status = :internal_server_error)
    ...
    error = Rails.env.production? ? t("generic_error") : exception.message
  ...
  def render_error(error, status = :unprocessable_entity, context = {})
    error_message =
      if error.is_a?(StandardError)
        has_record = error.respond_to?(:record)
        has_record ? error.record&.errors&.full_messages&.to_sentence : error.message
      else
        error
      end
  ...
  def authenticate_user_using_x_auth_token
    ...
    is_valid_token = user && auth_token &&
